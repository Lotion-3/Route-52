...
def _load_cookie_pool() -> list[dict]:
    ...
    try:
        ...
        remote = [{"cookies": e["cookies"], "ua": e["ua"], "created_at": e.get("created_at")}
                  for e in session_store.load_pool("target") if e.get("cookies")]
    except Exception as e:
        print(f"[Target] Supabase pool unavailable ({repr(e)[:100]}).", flush=True)
    pool = sorted(local + remote, key=_entry_epoch, reverse=True)
    print(f"[Target] Loaded {len(local)} local + {len(remote)} Supabase-pool "
    ...
    with _http_lock:
        if _http_session is None:
            ...
            if cached and _validate_http_session(cached):
                ...
                raise _ImpervaBlocked("pool/remote/disk all exhausted and ALLOW_BROWSER_WARM=0 "
